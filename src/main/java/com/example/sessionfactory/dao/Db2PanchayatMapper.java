...
import com.example.sessionfactory.config.Db2DatabaseConfig;
...
@Repository
public class Db2PanchayatMapper {
    private static String prefix="com.example.sessionfactory.dao.Db2PanchayatMapper.";

    public void createPanchayatTable() {
        try(SqlSession sqlSession = Db2DatabaseConfig.getSqlSessionFactory().openSession(true);) {
    ...
    public List<Panchayat> findAll(){
        try (SqlSession session=Db2DatabaseConfig.getSqlSessionFactory().openSession(true)){
